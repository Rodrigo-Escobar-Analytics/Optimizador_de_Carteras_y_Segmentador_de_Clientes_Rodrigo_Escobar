...
ui <- fluidPage(
  ...
  titlePanel("Optimization Dashboard MVP (Level 1)"),
...
server <- function(input, output, session) {
  ...
  output$kpis <- renderUI({
    ...
    n_accts <- nrow(accts)
    aum <- (sum(accts$MONTO_EN_CUSTODIA_TOTAL, na.rm = TRUE)/1000)
    ...
    tagList(
      ...
      fluidRow(
        ...
        column(4, tags$div(tags$b("Monto en custodia total (MILES DE PESOS): "), comma(aum, accuracy = 1))),
      ),
  ...
  actions_data <- reactive({
    ...
    if (input$cliente != "All" && input$cuenta != "All") {
      ...
      sub %>%
        filter(FLAG_ACCION_Q %in% c("COMPRAR", "VENDER")) %>%
        select(
