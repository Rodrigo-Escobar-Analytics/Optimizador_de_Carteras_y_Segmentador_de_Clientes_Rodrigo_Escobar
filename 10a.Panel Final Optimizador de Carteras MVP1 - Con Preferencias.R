...
DATA_PATH <- "FILES/OUTPUT/09_INDICADORES_CARTERA_CON_PREFERENCIA_SEGMENTO.CSV"
...
acct_df <- df %>%
  ...
  select(
    ...
    MSG_PREF, FLAG_MEJORA_RIESGO_ANUAL, FLAG_MEJORA_RETORNO_ANUAL, MONTO_TOTAL_ACCIONES,
...
ui <- fluidPage(
  ...
  titlePanel("Dashboard de Carteras Optimizadas - Version Con Preferencias y Segmentos."),
  ...
  ",
    ...
    h3("Optimización con Preferencias"),
    
    p("Este panel compara la cartera actual del cliente con una propuesta optimizada que incorpora las preferencias declaradas en distintas instancias de contacto. El objetivo es mejorar la relación entre retorno esperado y riesgo, entregando una recomendación más personalizada, interpretable y accionable."),
    
    h4("Cómo leerlo", style = "margin-top: 20px;"),
    
    tags$ul(
      tags$li("Revise primero los ", tags$b("indicadores resumen"), " para identificar cambios en retorno, riesgo y cantidad de movimientos sugeridos."),
      tags$li("Use luego el ", tags$b("gráfico riesgo-retorno"), " para visualizar si la cartera optimizada mejora su posición frente a la actual."),
      tags$li("Finalmente, consulte la ", tags$b("tabla detalle"), " para revisar qué acciones comprar, vender o mantener, incluyendo las preferencias agregadas."),
      tags$li("Puede descargar los archivos para facilitar la revisión y complementar el análisis con la información de ", tags$b("segmentos"), ".")
    ),
    
    h4("Resultado esperado", style = "margin-top: 20px;"),
    
    p("Se espera una mejora en retorno esperado, con un nivel de riesgo controlado o más eficiente, junto con una propuesta mejor alineada con los intereses del cliente. Esto permite priorizar oportunidades comerciales y respaldar una recomendación más clara, consistente y trazable."),
    
    p(
      tags$b("Importante: "),
      "esta herramienta es de uso referencial y debe complementarse siempre con criterio financiero. Al tratarse de un modelo estadístico, puede presentar un margen de error y no reemplaza la evaluación profesional."
    )
  ),
  ...
  resumen_cartera <- reactive({
    df %>%
      ...
      summarise(
        ...
        MONTO_CUSTODIA_CUENTA = sum(MONTO_EN_CUSTODIA, na.rm = TRUE),
